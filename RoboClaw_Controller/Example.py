...
def getInfo():
	...
	if temp[0]:
		...
		#returns  [Temperature(2 bytes), Checksum]
	else:
	...
	if mbat[0]:
		...
		#returns  [Value.Byte1, Value.Byte0, Checksum]
	else:
	...
	if current[0]:
		...
		#returns [M1Cur.Byte1, M1Cur.Byte0, M2Cur.Byte1, M2Cur.Byte0, Checksum]

	batt  = roboclaw.ReadLogicBatterySettings()
	if batt[0]:
		print "Logic Battery Min: ",batt[1]/10.0," Max: ",batt[2]/10.0
		#returns [Min(2 bytes), Max(2 bytes), Checksum]
	...
	if pid[0]:
		...
		#returns [P(4 bytes), I(4 bytes), D(4 bytes), MaxI(4 byte), Deadzone(4 byte), MinPos(4 byte), MaxPos(4 byte), Checksum]
